...
The goal is to allow registering of functions as command handlers by prefixing them with a decorator.

telegram.Updater.register_handler expects a function with arguments like
  ` bot, update, cmd_args, **kwargs `.

Such functions are generated by the pull_bot_and_update() functional wrapper, which takes a function "bound_method".

The "pull_bot_and_update::wrapped" function will set up the arguments as used by bound_method, and finally call it.
Effectively, pull_bot_and_update takes any function with arguments
` string `, ` args `, ` update ` or ` query `, along with the pass_*=boolean flags to decide which of these are needed,
and builds an interface which can be called by the telegram bot api.


This is used by the actual decorator command::command_decorator, which itself is created and configured
by the command function.

by writing
@command(parse_args=True)
def fn(self, args):
    ...

first, the command function is executed which generates
@command_decorator
def fn(self, args):
    ...

but the arguments passed to @command still exist in the scope of command_decorator, because closures.
Then, command_decorator is run on fn, which sets some member variables of fn and finally wraps it in
handle_herberrors() which will catch any exception leaving the command handler and inform the user.

one of the set member variables is the function fn.command_handler(), which just calls pull_bot_and_update(), wraps the
resulting command handler function in a CommandHandler object and returns it, which is finally passed to
the python-telegram-bot api when calling register_bert(fn.__self__) in core.py
